 ...
 */
public class BoxNotify extends Fragment {
    ...
    private ArrayList<BoxNotifyDB> BoxNotifyDBList;
    ...
    private void loadFunc() {
        ...
        JSONObject json = sr.getJson(conf.url_boxList, params);
        BoxNotifyDBList = new ArrayList<>();
        if (json != null) {
            try {
                if (json.getBoolean(conf.res)) {
                    ...
                    if (dataJsonArray.length() != 0) {
                        for (int i = 0; i < dataJsonArray.length(); i++) {
                            ...
                            String service = c.getString(conf.tag_service);
                            String idService = c.getString(conf.tag_idService);
                            ...
                            boolean main = c.getBoolean(conf.tag_main);
                            String usernameMain = c.getString(conf.tag_usernameMain);
                            BoxNotifyDB box = new BoxNotifyDB(service, idService, name, token, username, status, main, usernameMain);
                            BoxNotifyDBList.add(box);
                        }
                    }
                }
            ...
            BoxNotifyAdapterList adapter = new BoxNotifyAdapterList(getActivity(), BoxNotifyDBList, BoxNotify.this);
